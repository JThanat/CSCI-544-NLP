...
for form in lemma_count.keys():
    ...
    for lemma in lemma_count[form].keys():
        cur_max_lemma = lemma_max.get(form, None)
        if cur_max_lemma is None or lemma_count[form][lemma] > lemma_count[form][cur_max_lemma]:
    ...
    cur_lemma_keys_count = len(lemma_count[form].keys())

    if cur_lemma_keys_count > 1:
        training_counts['Ambiguous types'] += 1
    else:
        training_counts['Unambiguous types'] += 1

    ambi_most_common_token = 0
    for lemma in lemma_count[form].keys():
        training_counts['Wordform tokens'] += lemma_count[form][lemma]
        if cur_lemma_keys_count > 1:
            training_counts['Ambiguous tokens'] += lemma_count[form][lemma]
            ambi_most_common_token = max(ambi_most_common_token, lemma_count[form][lemma])
        else:
            training_counts['Unambiguous tokens'] += lemma_count[form][lemma]

        if lemma == form:
            training_counts['Identity tokens'] += lemma_count[form][lemma]

    training_counts['Ambiguous most common tokens'] += ambi_most_common_token

accuracies['Expected lookup'] = (training_counts['Unambiguous tokens'] + training_counts[
    'Ambiguous most common tokens']) / training_counts[
                                    'Wordform tokens']  ### Calculate expected accuracy if we used lookup on all items ###

accuracies['Expected identity'] = training_counts['Identity tokens'] / training_counts[
    'Wordform tokens']  ### Calculate expected accuracy if we used identity mapping on all items ###
...
try:
    ...
    accuracies['Identity'] = 'Undefined'

accuracies['Overall'] = (test_counts['Lookup match'] + test_counts['Identity match']) / (
test_counts['Total test items'])  ### Calculate overall accuracy ###
